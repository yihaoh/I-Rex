...
/********************************************************************************
  IID Comparison Functions
 ********************************************************************************/

PG_FUNCTION_INFO_V1(is_iid_ge);
Datum is_iid_ge(PG_FUNCTION_ARGS) {
    HeapTupleHeader rec1 = PG_GETARG_HEAPTUPLEHEADER(0);
    HeapTupleHeader rec2 = PG_GETARG_HEAPTUPLEHEADER(1);
    ArrayType *bool_arr = PG_GETARG_ARRAYTYPE_P(2);

    /* Get Tuple Descriptors for both records */
    Oid tupType1 = HeapTupleHeaderGetTypeId(rec1);
    int32 tupTypmod1 = HeapTupleHeaderGetTypMod(rec1);
    TupleDesc tupdesc1 = lookup_rowtype_tupdesc(tupType1, tupTypmod1);

    Oid tupType2 = HeapTupleHeaderGetTypeId(rec2);
    int32 tupTypmod2 = HeapTupleHeaderGetTypMod(rec2);
    ...
    if (tupdesc1->natts != tupdesc2->natts) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 errmsg("records have different number of columns")));
    }

    int natts = tupdesc1->natts;

    /* 2. Process boolean array */
    Oid elemtype = ARR_ELEMTYPE(bool_arr);
    if (elemtype != BOOLOID) {
        ReleaseTupleDesc(tupdesc1);
        ReleaseTupleDesc(tupdesc2);
        ereport(ERROR,
                (errcode(ERRCODE_DATATYPE_MISMATCH),
                 errmsg("third argument must be a boolean array")));
    }
    ...
    deconstruct_array(bool_arr, elemtype, elmlen, elmbyval, elmalign,
                      &elems, &nulls, &nelems);
    ...
    if (nelems != natts) {
        ...
        ereport(ERROR,
                (errcode(ERRCODE_ARRAY_SUBSCRIPT_ERROR),
                 errmsg("boolean array length must match record column count")));
    }

    /* Wrap Headers into HeapTupleData to use heap_getattr */
    HeapTupleData tmptup1, tmptup2;
    tmptup1.t_len = HeapTupleHeaderGetDatumLength(rec1);
    ItemPointerSetInvalid(&(tmptup1.t_self));
    tmptup1.t_tableOid = InvalidOid;
    tmptup1.t_data = rec1;

    tmptup2.t_len = HeapTupleHeaderGetDatumLength(rec2);
    ItemPointerSetInvalid(&(tmptup2.t_self));
    tmptup2.t_tableOid = InvalidOid;
    tmptup2.t_data = rec2;

    bool result = true; /* Default to true if all elements are exactly equal */

    /* 3. Iterate and compare values */
    for (int i = 0; i < natts; i++) {
        Form_pg_attribute att1 = TupleDescAttr(tupdesc1, i);
        Form_pg_attribute att2 = TupleDescAttr(tupdesc2, i);

        /* Ignore logically dropped columns */
        if (att1->attisdropped || att2->attisdropped)
            continue;

        /* Enforce type matching at the same position */
        if (att1->atttypid != att2->atttypid) {
            ReleaseTupleDesc(tupdesc1);
            ReleaseTupleDesc(tupdesc2);
            ereport(ERROR,
                    (errcode(ERRCODE_DATATYPE_MISMATCH),
                     errmsg("type mismatch at column position %d", i + 1)));
        }

        if (nulls[i]) {
            ereport(ERROR,
                    (errcode(ERRCODE_NULL_VALUE_NOT_ALLOWED),
                     errmsg("boolean array cannot contain null values")));
        }

        bool isnull1, isnull2;
        Datum val1 = heap_getattr(&tmptup1, i + 1, tupdesc1, &isnull1);
        Datum val2 = heap_getattr(&tmptup2, i + 1, tupdesc2, &isnull2);

        bool is_desc = DatumGetBool(elems[i]);
        int cmp_result = 0;

        /* 4. NULL Handling */
        if (isnull1 && isnull2) {
            /* Both are NULL -> considered equal, proceed to next value */
            continue;
        } else if (isnull1) {
            /* PG Default: NULL is considered greater than non-NULL */
            cmp_result = 1;
        } else if (isnull2) {
            cmp_result = -1;
        } else {
            /* Use TYPECACHE_CMP_PROC_FINFO to ensure cmp_proc_finfo 
             * is safely initialized in memory before we call it. 
             */
            TypeCacheEntry *typentry = lookup_type_cache(att1->atttypid, TYPECACHE_CMP_PROC_FINFO);
            if (!OidIsValid(typentry->cmp_proc))
            {
                ...
                ereport(ERROR,
                        (errcode(ERRCODE_UNDEFINED_FUNCTION),
                         errmsg("could not identify a comparison function for type %u", att1->atttypid)));
            }

            FmgrInfo *cmp_func = &typentry->cmp_proc_finfo;
            /* Returns -1 (less), 0 (equal), or 1 (greater) */
            cmp_result = DatumGetInt32(FunctionCall2Coll(cmp_func, att1->attcollation, val1, val2));
        }

        /* Apply ASC / DESC rules based on boolean array */
        if (is_desc)
            cmp_result = -cmp_result;

        /* Evaluate current field comparison */
        if (cmp_result > 0) {
            result = true;
            break; /* Early exit, rec1 > rec2 */
        } else if (cmp_result < 0) {
            result = false;
            break; /* Early exit, rec1 < rec2 */
        }
        /* If cmp_result == 0, values are equal, proceed to next loop iteration */
    }
...
PG_FUNCTION_INFO_V1(is_iid_le);
Datum is_iid_le(PG_FUNCTION_ARGS) {
    HeapTupleHeader rec1 = PG_GETARG_HEAPTUPLEHEADER(0);
    HeapTupleHeader rec2 = PG_GETARG_HEAPTUPLEHEADER(1);
    ArrayType *bool_arr = PG_GETARG_ARRAYTYPE_P(2);
...
PG_FUNCTION_INFO_V1(is_iid_greater);
Datum is_iid_greater(PG_FUNCTION_ARGS) {
    HeapTupleHeader rec1 = PG_GETARG_HEAPTUPLEHEADER(0);
    HeapTupleHeader rec2 = PG_GETARG_HEAPTUPLEHEADER(1);
    ArrayType *bool_arr = PG_GETARG_ARRAYTYPE_P(2);
    ...
    bool result = false; /* Default to false if all elements are exactly equal */
...
PG_FUNCTION_INFO_V1(is_iid_less);
Datum is_iid_less(PG_FUNCTION_ARGS) {
    HeapTupleHeader rec1 = PG_GETARG_HEAPTUPLEHEADER(0);
    HeapTupleHeader rec2 = PG_GETARG_HEAPTUPLEHEADER(1);
    ArrayType *bool_arr = PG_GETARG_ARRAYTYPE_P(2);
...
PG_FUNCTION_INFO_V1(is_iid_equal);
Datum is_iid_equal(PG_FUNCTION_ARGS) {
    HeapTupleHeader rec1 = PG_GETARG_HEAPTUPLEHEADER(0);
    HeapTupleHeader rec2 = PG_GETARG_HEAPTUPLEHEADER(1);
    ...
    for (int i = 0; i < natts; i++) {
        ...
        /* Type mismatch, early exit with false */
        if (att1->atttypid != att2->atttypid) {
            result = false;
            break;
        }
